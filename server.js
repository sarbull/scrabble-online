...
io.sockets.on('connection', function (socket) {
  ...
  // on the frontend you sent the params like so: 
  // socket.emit('data', { letter: 'a', details: 0 });
  socket.on('data', function (data) {
    // this broadcasts the message to all connected users, except the one who sent the message
    socket.broadcast.emit('scrabble_data', data);
    console.log('client sent scrabble data', data);
  });
});
